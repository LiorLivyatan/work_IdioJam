"""
Utils for the PARSEME VID task.
"""
...
# type 3
PROMPTS = {
    "system": """You are a professional linguist specializing in figurative language and your task is to analyse sentences that may contain a verbal idiom. 
Head verb is the main verb in a phrase that determines the grammatical structure and meaning of its dependents.Verbal idioms constitute a universal category. A verbal idiom (VID) has at least two lexicalized components including a head verb and at least one of its dependents. The dependent can be of different types.
Idioms typically have both a literal and an idiomatic reading. They are often semantically totally non-compositional, i.e. none of their lexicalized components retains any of their original meanings.
Mark VIDs only when their usage in the context is idiomatic/figurative and let literal meanings remain unmarked.
...
def get_data(**kwargs) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    test_file = os.path.join(test_dir, f"{lang}/test_vid.jsonl")
    train_file = os.path.join(train_dir, f"{lang}/train_vid.jsonl")
...
def process_responses(
    ...
    for res in results:
        ...
        for resp in res["responses"]:
            try:
                if "parsed" in resp:
                    if "vids" in resp["parsed"]:
                        _mwes = resp["parsed"]["vids"]
