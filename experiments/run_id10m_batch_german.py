"""
Batch runner for updated id10m German experiments.

For each experiment in the archive:
  1. Run inference on the 5 corrected sentences (text cleaned in current dataset)
  2. Merge with the 132 unchanged responses from the archive run
  3. Evaluate on the full 137-sentence FINAL dataset

All outputs land in:
  results/id10m/german/updated/{model}/{prompt}/seed_{N}/
      responses_5.json      # new responses for the 5 corrected sentences
      responses.json        # merged 137-row dataset
      config.yaml
      metrics.json
      results.tsv
...
Usage:
    python experiments/run_id10m_batch_german.py               # run all 38 experiments
    python experiments/run_id10m_batch_german.py --dry_run      # print plan only
    python experiments/run_id10m_batch_german.py --filter gpt-4o-mini
    python experiments/run_id10m_batch_german.py --skip_inference  # merge+eval only
"""
...
RESULTS_DIR     = REPO_ROOT / "results" / "id10m" / "german"
ARCHIVE_DIR     = RESULTS_DIR / "archive"
UPDATED_DIR     = RESULTS_DIR / "updated"
UPDATED_SUMMARY = UPDATED_DIR / "results_summary.csv"

# Pre-built JSON data files (see data/raw_id10m_data/german/)
FINAL_JSON   = REPO_ROOT / "data" / "raw_id10m_data" / "german" / "id10m_german_FINAL.json"
SUBSET_JSON  = REPO_ROOT / "data" / "raw_id10m_data" / "german" / "id10m_german_subset5.json"
TRAIN_TSV    = REPO_ROOT / "data" / "id10m" / "trainset" / "german.tsv"
...
def load_train_data() -> pd.DataFrame:
    """Load German train set from BIO TSV for few-shot examples."""
...
def run_inference_on_subset(
    ...
    """Run the model on the 5-sentence subset. Returns response dicts."""
...
def build_merged_responses(
    ...
    """
    Build 137-row merged list in test_df order:
    - 132 rows: reused from archive (exact sentence match, old format normalized)
    - 5 rows: from new inference
    """
...
def evaluate_and_save(
    ...
    """Run process_responses on merged 137 rows and save all outputs."""
...
def update_summary_csv(model: str, prompt_cfg: dict, seed: int, metrics: dict):
    ...
    new_row = {
        ...
        "sc_runs":        prompt_cfg["sc_runs"],
        "temperature":    prompt_cfg["temperature"],
        "accuracy":       round(ger.get("accuracy", 0.0), 6) if isinstance(ger.get("accuracy"), float) else "",
        "precision":      round(ger.get("precision", 0.0), 6) if isinstance(ger.get("precision"), float) else "",
        "recall":         round(ger.get("recall", 0.0), 6) if isinstance(ger.get("recall"), float) else "",
        "f1":             round(ger.get("f1", 0.0), 6) if isinstance(ger.get("f1"), float) else "",
...
def main():
    ...
    parser.add_argument("--skip_inference", action="store_true", help="Merge+eval only; requires responses_5.json")
    ...
    logger.info("Loading test data (137 sentences)...")
    test_df = load_json_data(FINAL_JSON)
    logger.info(f"Test data: {len(test_df)} rows")

    logger.info("Loading subset data (5 corrected sentences)...")
    ...
    for run in runs:
        ...
        # --- Step 1: Inference on 5 corrected sentences ---
        if not args.skip_inference:
            ...
            save_json(new_responses, run_dir / "responses_5.json")
            logger.info(f"Saved responses_5.json ({len(new_responses)} rows)")
        else:
            path_5 = run_dir / "responses_5.json"
            if not path_5.exists():
                logger.error(f"--skip_inference set but {path_5} not found — skipping")
                continue
            with open(path_5, encoding="utf-8-sig") as f:
                new_responses = json.load(f)
            logger.info(f"Loaded existing responses_5.json ({len(new_responses)} rows)")

        # --- Step 2: Merge 132 archive + 5 new = 137 ---
        logger.info("Step 2: Merging archive (132) + new (5)...")
        merged = build_merged_responses(archive_dir, new_responses, test_df)

        if len(merged) != 137:
            logger.error(f"Expected 137 merged rows, got {len(merged)} — aborting this run")
            continue

        save_json(merged, run_dir / "responses.json")
        logger.info(f"Saved responses.json ({len(merged)} rows)")

        # --- Step 3: Evaluate on full 137-sentence set ---
        logger.info("Step 3: Evaluating on 137 sentences...")
        try:
